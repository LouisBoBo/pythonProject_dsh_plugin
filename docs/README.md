...
├── 10-token用量统计/          ← LLM 用量计量（口径冻结）
├── 11-专家技能连接器/         ← 对标 WorkBuddy 专家/技能/连接器
...
## 8. 专家 · 技能 · 连接器 → [`11-专家技能连接器/`](./11-专家技能连接器/)

> 独立 DSH 插件（建议 `@zhongruan/dsh-esc`），**零改** WorkBuddy，**不改** 现有公司插件。  
> 对照：腾讯 WorkBuddy「专家-技能-连接器」；PCB 售后/测试/工艺 + Dify/MES。

| 文档 | 说明 | 状态 |
| --- | --- | --- |
| [专家技能连接器插件-可行性实施方案.md](./11-专家技能连接器/专家技能连接器插件-可行性实施方案.md) | **主方案**：三层解耦、P0 落地点、隔离与验收 | **已确认开工**（双场景 P0、MES 真接口、Dify 降级、并存 mes_pcb） |
