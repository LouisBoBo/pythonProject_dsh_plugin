...
export function apply(ctx) {
    ctx.tools.register(defineTool({
        name: 'pcb_8d_demo',
        description: '【一键生成完整模拟 8D 报告】当用户要演示 8D、要一份完整样例、全模拟业务数据、输出完整报告时调用。' +
        ...
        output: {
            ...
            render: (_args, value) => [
                {
                    type: 'text',
                    text: `已生成完整模拟 8D：${value.id}（${value.filledSteps}/9）\n` +
    ...
    ctx.tools.register(defineTool({
        name: 'pcb_8d_create',
        description: '【新建 PCB 8D 报告草稿】当用户要开 8D、写 8D 报告、做质量纠正报告时调用。' +
    ...
    ctx.tools.register(defineTool({
        name: 'pcb_8d_fill',
        description: '【补全 8D 某一步】用户补充 D0～D8 任一章节内容时调用。' +
        ...
        output: {
            ...
            render: (_args, value) => [
                {
                    type: 'text',
                    text: `已更新 ${value.id} → ${value.step} ${value.stepTitle}\n` +
    ...
    ctx.tools.register(defineTool({
        name: 'pcb_8d_export',
        description: '【导出 8D 为 Markdown】参数 reportId。写入本机 .md 并返回全文。',
        parameters: {
            reportId: {
                type: 'string',
                required: true,
        ...
        output: {
            ...
            render: (_args, value) => [
                {
                    type: 'text',
                    text: `已导出 ${value.id}（${value.filledSteps}/9）\n` +
                        `文件：${value.filePath}\n\n` +
                        value.markdown,
                },
            ],
    ...
    console.log('[pcb-8d] 插件已加载：pcb_8d_demo / create / fill / get / list / export；目录 ' +
        storeDir());
}
