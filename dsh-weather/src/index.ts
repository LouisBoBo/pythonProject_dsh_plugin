...
async function geocodeCity(city: string): Promise<GeoHit> {
  const q = city.trim()
  if (!q) throw new Error('城市名不能为空')
  ...
  if (!hit) throw new Error(`找不到城市「${q}」，请换常用地名再试（如 深圳、Beijing）`)
  return hit
}
...
async function fetchCurrentWeather(city: string) {
  ...
  const res = await fetch(url)
  if (!res.ok) throw new Error(`天气接口失败 HTTP ${res.status}`)
  ...
  if (!cur || cur.temperature_2m == null) throw new Error('天气接口未返回实况数据')
...
async function fetchDailyForecast(city: string, days: number) {
  const geo = await geocodeCity(city)
  ...
  const res = await fetch(url)
  if (!res.ok) throw new Error(`预报接口失败 HTTP ${res.status}`)
  const data = (await res.json()) as {
    daily?: {
      time?: string[]
      weather_code?: number[]
  ...
  if (!daily?.time?.length) throw new Error('预报接口未返回日数据')
...
export function apply(ctx: Context) {
  ctx.tools.register(
    defineTool({
      name: 'weather_query',
      description:
        '【查天气实况】当用户问某城市现在天气、气温、湿度、风力时必须调用。' +
        ...
        render: (_args, value) => [
          {
            type: 'text',
            text:
              `${value.city} 实况：${value.weatherText}，` +
  ...
  ctx.tools.register(
    defineTool({
      ...
      description:
        '【查未来几天天气预报】当用户问某城市未来几天天气、最高最低温、会不会下雨时调用。' +
      ...
      output: {
        ...
        render: (_args, value) => {
          const lines = (value.days ?? []).map(
            (d) =>
              `${d.date ?? ''} ${d.weatherText ?? ''} ${d.tempMinC ?? 0}～${d.tempMaxC ?? 0}°C 降水${d.precipitationMm ?? 0}mm`,
          )
          return [
            {
              type: 'text',
