...
import { createNoteEditorChrome } from './web-note-editor-chrome.js';
 ...
 */
const bodyEditorEvents = {
    dragstart: (view, event) => {
        if (view.state.selection.empty || isNodeSelection(view.state.selection))
            return false;
...
export function plainTextToDocument(text) {
    return {
        type: 'doc',
...
export function plainTextFromDocument(document) {
    return (document.content ?? []).map(paragraph => (paragraph.content ?? []).map(node => node.text ?? '').join('')).join('\n');
}
//# sourceMappingURL=web-note-editor.js.map