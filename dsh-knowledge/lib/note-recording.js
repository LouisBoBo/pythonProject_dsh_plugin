...
export function createNoteRecording(provider) {
    const read = async (id, signal) => {
        const { node, content } = await provider.readNote(id, signal);
        if (!node?.editable || node.kind === 'folder')
            throw new Error('记录笔记不存在或不是可编辑文本');
        if (content.byteLength > 200_000)
            throw new Error('记录笔记过大，请先整理后继续');
    ...
    return {
        ...
        read,
        async update(id, value, expectedRevision, signal) {
            if (typeof value !== 'string' || Buffer.byteLength(value) > 200_000)
                throw new Error('记录内容超过上限');
            const current = await read(id, signal);
            if (current.revision !== expectedRevision)
                throw new Error('笔记已变化，请重新读取后再更新');
...
//# sourceMappingURL=note-recording.js.map