...
export class KnowledgeNoteHandleCodec {
    ...
    constructor(secret) {
        this.secret = secret;
        if (secret.length < 32)
            throw new Error('knowledge note handle secret must contain at least 32 bytes');
    }
    encode(sessionId, noteId) {
        const payload = Buffer.from(JSON.stringify({ v: 1, sessionId, noteId })).toString('base64url');
