...
export function registerRemoteManagementProxy(ctx, prefix, current, localService) {
    const webServer = ctx.webServer ?? ctx.get('webServer');
    if (webServer === undefined)
        throw new Error('remote knowledge management requires the DSH webServer service');
    return webServer.register({
        kind: 'prefix',
        path: prefix,
        handler: async (req, res) => {
            try {
                await dispatch(prefix, current(), localService, req, res);
            }
            catch (error) {
                sendError(res, error);
            }
...
async function dispatch(prefix, settings, localService, req, res) {
    ...
    if (relative === 'service') {
        ...
        if (method !== 'PUT')
            throw httpError(405, '不支持此本机设置请求方法。');
        const body = JSON.parse((await readBody(req, MAX_JSON_REQUEST_BYTES))?.toString('utf8') ?? '{}');
        if (body.publicApiEnabled !== undefined)
            throw httpError(409, '请在中央 DSH 的知识库管理台中修改远程 API 状态。');
    ...
    if (!['GET', 'POST', 'PUT', 'PATCH', 'DELETE'].includes(method))
        throw httpError(405, '不支持此远程管理请求方法。');
    ...
    const knowledgeImportRoute = /(?:^|\/)knowledge-bases\/[^/]+\/import(?:\?|$)/.test(relative);
    const knowledgeOriginalRoute = /(?:^|\/)entries\/[^/]+\/(?:original|preview)(?:\?|$)/.test(relative);
    const binaryRoute = noteContentRoute || knowledgeImportRoute || knowledgeOriginalRoute;
    const body = method === 'GET' ? undefined : await readBody(req, binaryRoute ? MAX_NOTE_FILE_BYTES : MAX_JSON_REQUEST_BYTES);
    ...
    try {
        ...
        const maximumResponseBytes = binaryRoute ? MAX_NOTE_FILE_BYTES : MAX_JSON_RESPONSE_BYTES;
        const declared = Number(response.headers.get('content-length') ?? 0);
        if (Number.isFinite(declared) && declared > maximumResponseBytes)
            throw httpError(502, '中央知识库响应过大。');
...
function assertManagementRequest(req) {
    if (req.headers['x-dsh-knowledge-client'] !== 'management-web')
        throw httpError(401, '缺少知识库管理客户端标识。');
    const fetchSite = req.headers['sec-fetch-site'];
    if (fetchSite !== undefined && fetchSite !== 'same-origin' && fetchSite !== 'none')
        throw httpError(403, '已拒绝跨站知识库管理请求。');
    ...
    try {
        ...
        throw httpError(403, '知识库管理请求来源无效。');
    }
    ...
    if (expectedHost === undefined || originHost.toLowerCase() !== expectedHost.toLowerCase())
        throw httpError(403, '已拒绝跨域知识库管理请求。');
}
async function readBody(req, maximumBytes) {
    const declared = Number(req.headers['content-length'] ?? 0);
    if (Number.isFinite(declared) && declared > maximumBytes)
        throw httpError(413, '远程管理请求内容过大。');
    const chunks = [];
    let size = 0;
...
//# sourceMappingURL=management-proxy.js.map