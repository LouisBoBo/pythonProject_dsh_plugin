...
export function createKnowledgeTrackingService(provider) {
    return {
        async record(agent, input, signal) {
            ...
            if (selected.outcome !== undefined)
                return { storage: 'local', outcome: selected.outcome };
            const mount = selected.mount;
            if (mount === undefined)
                return { storage: 'local', outcome: 'not-mounted' };
            if (mount.writeMode === 'none')
                return { storage: 'local', outcome: 'not-writable', knowledgeBaseId: mount.knowledgeBaseId };
        ...
        async list(agent, query = '', limit = 20, signal) {
            ...
            const batches = await mapConcurrent(mounts, 4, async (mount) => {
                ...
                return documents.flatMap(document => {
                    ...
                    if (!baseReference || !documentReference || baseReference.includes(']') || documentReference.includes(']'))
                        return [];
...
function selectMount(mounts, reference) {
    ...
    if (candidates.length === 0)
        return { outcome: writable.length > 0 ? 'not-writable' : 'not-mounted' };
    if (candidates.length > 1)
        return { outcome: 'ambiguous' };
    return { mount: candidates[0] };
}
async function findEntry(provider, agent, mount, title, signal) {
...
function observationMarkdown(input) {
