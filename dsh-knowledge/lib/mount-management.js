...
import { normalizeKnowledgeMountDraft } from './domain.js';
...
export function createKnowledgeMountManagement(provider, connectionRevision = () => 0) {
    const read = async (input, signal) => {
        ...
        for (let offset = 0; offset < targets.length; offset += 8) {
            ...
            if (connectionRevision() !== connection)
                throw new Error('知识库连接已切换，请重新读取挂载后重试');
        }
    ...
    return {
        ...
        read,
        async configure(targets, expectedRevision, input, signal, beforeWrite) {
            const connection = connectionRevision();
            const current = await read(targets, signal);
            if (!expectedRevision || current.revision !== expectedRevision)
                throw new Error('知识库挂载已变化，请重新读取后重试');
            if (typeof input.enabled !== 'boolean' || typeof input.recallEnabled !== 'boolean')
                throw new Error('enabled 和 recallEnabled 必须为布尔值');
...
function validateTargets(targets) {
    if (!Array.isArray(targets) || targets.length < 1 || targets.length > 100)
        throw new Error('挂载目标数量必须在 1 到 100 之间');
    const seen = new Set();
    return targets.map(target => {
        if (!target || (target.kind !== 'project' && target.kind !== 'session') || typeof target.id !== 'string' || !target.id.trim())
            throw new Error('挂载目标无效');
        const value = { kind: target.kind, id: target.id.trim() };
        const key = JSON.stringify(value);
        if (seen.has(key))
            throw new Error('挂载目标重复');
        seen.add(key);
        return value;
    });
}
//# sourceMappingURL=mount-management.js.map