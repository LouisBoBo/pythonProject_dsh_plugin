...
export function documentLifecycleTool(provider, codec) {
    return {
        ...
        async execute(raw, exec) {
            ...
            if (expectedVersion === undefined)
                throw new Error('请先读取文档并提供 expectedVersion');
            if (args.state !== 'resolved' && args.state !== 'complete')
                throw new Error('state 必须是 resolved 或 complete');
            if (typeof args.wholeDocument !== 'boolean')
                throw new Error('必须明确 wholeDocument');
            const { entry, mount } = await readMountedKnowledge(provider, agent, handle, codec, exec.signal);
            if (mount.writeMode === 'none')
                throw new Error('本会话对此知识库只有只读权限');
            if (entry.version !== expectedVersion)
                throw new Error('文档版本已变化，请重新读取并确认范围');
            const proposal = lifecycleProposal(entry, {
                ...
                ...args.oldText === undefined ? {} : { oldText: args.oldText },
                ...args.newText === undefined ? {} : { newText: args.newText },
            ...
            if (current.mount.writeMode === 'none')
                throw new Error('知识库写权限已撤回');
            if (current.entry.version !== expectedVersion)
                throw new Error('文档版本已变化，请重新读取');
...
//# sourceMappingURL=document-lifecycle-tool.js.map