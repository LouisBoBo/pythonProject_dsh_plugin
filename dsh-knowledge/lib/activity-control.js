...
export function registerKnowledgeActivityControl(ctx, provider) {
    const webServer = ctx.webServer ?? ctx.get('webServer');
    if (webServer === undefined)
        throw new Error('knowledge activity panel requires the DSH webServer service');
    return webServer.register({
        kind: 'prefix',
        path: KNOWLEDGE_ACTIVITY_PATH,
        handler: async (req, res) => {
            try {
                await dispatch(req, res, provider);
            }
            catch (error) {
                sendError(res, error);
            }
...
async function dispatch(req, res, provider) {
    if (req.method !== 'GET') {
    ...
    let segments;
    try {
        segments = relative ? relative.split('/').map(decodeURIComponent) : [];
    }
    catch {
        throw httpError(400, '知识侧栏路径格式无效。');
    }
    ...
    if (segments[0] === 'notes' && segments[1] !== undefined && segments[2] === 'content' && segments.length === 3) {
        ...
        if (node === undefined)
            throw httpError(404, '这份笔记不存在。');
        if (!isEditableNoteNode(node))
            throw httpError(415, '该文件暂不支持在会话侧栏中预览。');
        if (node.size > MAX_ACTIVITY_NOTE_BYTES)
            throw httpError(413, '这份笔记较大，请在完整工作区中打开。');
...
//# sourceMappingURL=activity-control.js.map