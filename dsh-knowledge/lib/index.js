...
import { connectionSettingsBase, createConnectionProvider, loadStoredConnection, sameConnection, storeConnection, validateConnectionSettings, } from './connection.js';
import { Config as ConfigSchema, resolveConfig } from './config.js';
import { KNOWLEDGE_SETTINGS_NAMESPACE } from './constants.js';
import { registerKnowledgeControl } from './control.js';
...
import { RemoteProviderError } from './remote-provider.js';
...
import { WritebackQueue, WritebackDeferred } from './writeback/queue.js';
...
export function apply(ctx, config) {
    ...
    const trust = runtime.get('remoteSettingsTrust');
    if (trust !== undefined) {
        const sync = () => {
        ...
        sync();
        if (typeof trust.subscribe === 'function')
            runtime.effect(() => trust.subscribe(sync), 'dsh-knowledge: trusted share origin synchronization');
    }
    runtime.inject?.(['settings'], settingsRuntime => {
        settingsRuntime.settings?.register(KNOWLEDGE_SETTINGS_NAMESPACE, Schema.object({}), { base: {} });
    });
    const persistedServicePath = serviceSettingsPath(resolved.connectionPath);
    let publicApiEnabled = resolved.exposeApi;
    let clientSettings = { publicApiEnabled };
    try {
        const storedService = loadServiceSettings(persistedServicePath);
        if (storedService !== undefined) {
            clientSettings = storedService;
            publicApiEnabled = storedService.publicApiEnabled;
        }
    }
    catch (error) {
    ...
    if (resolved.backend === 'remote') {
        ...
        clientSettings = { ...clientSettings, publicApiEnabled: false };
    }
    ...
    const connectionProvider = (settings) => {
    ...
    const provider = providerRouter.provider;
    let activeConnection = initialConnection;
    let connectionChanging = false;
    const coordinator = new ExtractionCoordinator(runtime, provider, resolved, () => (clientSettings.writebackProvider && clientSettings.writebackModel
        ? { provider: clientSettings.writebackProvider, model: clientSettings.writebackModel }
        : undefined));
    const writebackStatuses = new Map();
    const writebackSources = new Map();
    const destination = () => activeConnection.backend === 'local' ? `local:${resolved.databasePath}` : `remote:${activeConnection.remoteUrl}`;
    const writebackQueue = resolved.extractionEnabled ? new WritebackQueue(resolved.writebackQueuePath, async (work, checkpoint, signal) => {
        if (connectionChanging)
            throw new WritebackDeferred('知识库连接正在切换，等待稳定后回写');
        if (work.destination !== destination())
            throw new Error('知识库连接已切换，旧回写仍绑定原目标；请恢复原连接后重试');
        const existing = await provider.extractionJob(work.snapshot.sourceKey, signal);
        if (existing?.status === 'completed')
            return statusFromExtractionJob(existing, false);
        if (existing?.status === 'failed' && existing.attempts >= 3)
            await provider.resetExtraction(work.snapshot.sourceKey, signal);
        const result = await coordinator.runSnapshot(work.snapshot, signal, checkpoint);
        if (result.status !== 'duplicate')
            return statusFromExtractionResult(result);
        const job = await provider.extractionJob(work.snapshot.sourceKey, signal);
        if (job?.status === 'completed')
            return statusFromExtractionJob(job, false);
        if (job?.status === 'running')
            throw new WritebackDeferred('上一次远端回写租约尚未释放，等待恢复');
    ...
    const updateConnection = (input) => {
        const pending = switching.then(async () => {
            ...
            if (sameConnection(next, activeConnection))
                return activeConnection;
            if (writebackQueue?.isRunning)
                throw connectionError(409, '回写正在执行，请完成后再切换知识库连接');
            if (resolved.connectionPath === undefined)
                throw connectionError(409, '当前插件没有配置持久化路径，无法保存连接。');
    ...
    const registerHttpSurfaces = (httpRuntime) => {
        ...
        const applyPublicApiRoute = (enabled) => {
            disposePublicApi?.();
            disposePublicApi = undefined;
            if (enabled) {
                if (managementProvider === undefined)
                    throw connectionError(409, '当前 DSH 没有可供远程访问的本地知识库。');
        ...
        const updateClientSettings = async (patch) => {
            const enabled = patch.publicApiEnabled ?? publicApiEnabled;
            if (enabled && activeConnection.backend !== 'local')
                throw connectionError(409, '请先把知识库来源切换为本地，再开启远程 API。');
            if (persistedServicePath === undefined)
                throw connectionError(409, '当前插件没有配置持久化路径，无法保存远程 API 状态。');
            ...
            if (!clearRoute && ((provider === undefined) !== (model === undefined)))
                throw connectionError(400, '本机回写模型需要同时选择提供方和模型。');
            if (!clearRoute && provider && model) {
                try {
                    await runtime.llm.resolveModelInfo(provider, model);
                }
                catch (error) {
                    throw connectionError(400, `当前客户端无法使用 ${provider} / ${model}：${error instanceof Error ? error.message : String(error)}`);
                }
            }
            ...
            try {
                ...
                if (publicApiEnabled !== previous.publicApiEnabled)
                    applyPublicApiRoute(previous.publicApiEnabled);
        ...
        let disposeManagementApi;
        const applyManagementApiRoute = () => {
        ...
        if (resolved.exposeWeb) {
            const disposeWeb = registerKnowledgeWeb(httpRuntime, resolved.webPath, LOCAL_MANAGEMENT_API_PREFIX, 'same-origin', managementEmbedToken);
        ...
        if (disposeWritebackLive)
            httpRuntime.effect(() => disposeWritebackLive, 'dsh-knowledge.writeback-live');
        if (disposeWritebackControl)
            httpRuntime.effect(() => disposeWritebackControl, 'dsh-knowledge.writeback-control');
            ...
            async handler(req, res) {
                try {
                    ...
                    if (!sessionId || !Number.isInteger(turn) || turn < 0)
                        throw connectionError(400, 'sessionId and a non-negative integer turn are required');
                    ...
                    if (req.method === 'POST') {
                        if (!writebackQueue)
                            throw connectionError(409, '知识库回写已停用');
        ...
        if (disposeWritebackStatus !== undefined)
            httpRuntime.effect(() => disposeWritebackStatus, 'dsh-knowledge.writeback-status');
        ...
        if (disposeModelCatalog !== undefined)
            httpRuntime.effect(() => disposeModelCatalog, 'dsh-knowledge.model-catalog');
    };
    ...
    if (resolved.extractionEnabled) {
        runtime.on('agent/turn-stopping', ({ agent, turn }) => {
            ...
            try {
                ...
                if (snapshot)
                    work = { snapshot, destination: destination() };
                if (work)
                    writebackQueue.enqueue(work);
                else
                    writebackQueue.completeEmpty(key, agent.session.id);
                writebackStatuses.delete(key);
                writebackSources.delete(key);
            }
            catch (error) {
...
function statusFromExtractionResult(result) {
    if (result.status === 'unmounted')
        return { status: 'completed', summary: '知识库回写 · 未挂载可写知识库', retryable: false };
    if (result.status === 'skipped')
        return { status: 'completed', summary: '知识库回写 · 当前回答无可提取内容', retryable: false };
    if (result.status === 'duplicate')
        return { status: 'completed', summary: '知识库回写 · 已处理', retryable: false };
    if (result.candidateCount === 0)
        return { status: 'completed', summary: '知识库回写 · 无需收录', retryable: false };
    return {
        status: 'completed',
...
function statusFromExtractionJob(job, retryable) {
    if (job.status === 'running')
        return { status: 'running', summary: '知识库回写 · 正在处理', retryable: false };
    ...
    if (completion.outcome === 'unmounted')
        return { status: 'completed', summary: '知识库回写 · 未挂载可写知识库', retryable: false };
    if (completion.outcome === 'skipped')
        return { status: 'completed', summary: '知识库回写 · 当前回答无可提取内容', retryable: false };
    if (completion.candidateCount === 0)
        return { status: 'completed', summary: '知识库回写 · 无需收录', retryable: false };
    ...
    for (const destination of completion.destinations) {
        ...
        if (destination.disposition === 'written')
...
function summarizeWritebackCounts(bases) {
...
function rememberWritebackStatus(statuses, sources, key, state) {
...
function publicConnectionError(error) {
    if (error instanceof RemoteProviderError) {
        if (error.status === 401)
            return connectionError(400, '客户端令牌无效或已被撤销。');
        if (error.status === 403)
            return connectionError(400, '客户端令牌没有读取该知识库的权限。');
        if (error.status === 404)
            return connectionError(400, '服务器地址不是有效的知识库 API 地址。');
        if (error.status === 0)
            return connectionError(400, '无法连接远程知识库，请检查地址、网络和证书。');
        return connectionError(400, `远程知识库验证失败（HTTP ${error.status}）。`);
    }
    if (error instanceof Error && typeof error.status === 'number')
        return error;
    if (error instanceof Error) {
        if (/server URL and client token|24 characters/i.test(error.message))
            return connectionError(400, '首次连接需要填写至少 24 个字符的客户端令牌。');
        if (/HTTPS/i.test(error.message))
            return connectionError(400, '远程知识库必须使用 HTTPS。');
        if (/timeout/i.test(error.message))
            return connectionError(400, '请求超时时间不正确。');
        if (/databasePath/i.test(error.message))
            return connectionError(409, '当前 DSH 没有可用的本地知识库。');
    }
