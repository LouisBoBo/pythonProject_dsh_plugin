...
import { contentHash, isKnowledgeType, normalizeTags } from './domain.js';
...
import { messageText } from './runtime.js';
export class ExtractionCoordinator {
    ...
    constructor(ctx, provider, config, clientRoute = () => undefined) {
        this.ctx = ctx;
        this.provider = provider;
        this.config = config;
        this.clientRoute = clientRoute;
    }
    async run(session, turn, parentSignal) {
        if (this.closing)
        ...
        if (snapshot === undefined)
            return this.completeEmpty(sourceKey, 'skipped', parentSignal);
        return this.runSnapshot(snapshot, parentSignal);
    }
    capture(session, turn) {
        return snapshotTurn(session, turn, this.config.extractionMaxInputChars);
    }
    async runSnapshot(snapshot, parentSignal, checkpoint) {
        if (this.closing)
            throw new Error('knowledge coordinator is shutting down');
        ...
        if (mounts.length === 0) {
            if (checkpoint?.load()?.length)
                throw new Error('回写计划的可写挂载已撤回，请恢复挂载后重试');
    ...
    async completeEmpty(sourceKey, status, signal) {
        const result = emptyResult(status);
        try {
            if (!await this.provider.claimExtraction(sourceKey, signal))
                return emptyResult('duplicate');
    ...
    async process(snapshot, mounts, signal, checkpoint) {
        if (!await this.provider.claimExtraction(snapshot.sourceKey, signal))
            return emptyResult('duplicate');
        try {
            ...
            for (const group of groups) {
                const existing = await findExistingEntries(this.provider, group.mounts, query, snapshot.projectId, signal);
                proposals.push(...await extractWithLlm(this.ctx, this.config, snapshot, group.mounts, existing, group.route, group.writebackPolicy, signal));
            }
            const plan = saved ?? coalesceDocumentProposals(proposals).flatMap(proposal => {
            ...
            if (checkpoint && saved === undefined)
                checkpoint.save(plan);
            // A slow model must not preserve a permission that was revoked meanwhile.
            if (checkpoint && plan.length > 0)
                mounts = (await this.provider.resolveMounts(snapshot.sessionId, snapshot.projectId, signal)).filter(mount => mount.writeMode !== 'none');
            if (checkpoint && this.provider.mode === 'remote' && plan.some(item => item.delivery === 'direct')) {
                ...
                if (protocol?.idempotentDirectWrites !== true)
                    throw new Error('远端知识库需升级以支持可靠回写与幂等重试；写入计划已保留');
            }
            ...
            for (const { proposal, delivery } of plan) {
                ...
                if (delivery === 'direct') {
                    if (mount.writeMode !== 'direct')
                        throw new Error('回写计划的直接写入权限已撤回，请恢复权限后重试');
                    const result = await this.provider.writeDirect(proposal, snapshot.sourceKey, signal);
                    if (result.outcome === 'finalized' || (result.outcome === 'duplicate' && !result.entry))
                        continue;
                    candidateCount += 1;
                    if (result.outcome === 'conflict') {
                        ...
                        if (result.candidate !== undefined)
                            destinations.push(pendingDestination(mount, result.candidate));
                    }
                    else {
                        directCount += 1;
                        if (counts !== undefined)
            ...
            try {
                await this.provider.failExtraction(snapshot.sourceKey, message);
            }
            catch { }
            throw error;
        }
...
async function findExistingEntries(provider, mounts, query, projectId, signal) {
...
function groupMountsByRoute(mounts, config, snapshot, clientRoute) {
    ...
    for (const mount of mounts) {
        ...
        const route = clientRoute ?? baseRoute ?? snapshot.route ?? (config.extractionProvider !== undefined && config.extractionModel !== undefined
            ...
            : undefined);
        if (route === undefined)
            throw new Error(`no extraction model route is available for knowledge base "${mount.base.name}"`);
        const key = `${route.provider}\u0000${route.model}\u0000${mount.base.writebackPolicy}`;
        const group = groups.get(key);
        if (group === undefined)
            groups.set(key, { route, writebackPolicy: mount.base.writebackPolicy, mounts: [mount] });
        else
...
function pendingDestination(mount, candidate) {
    return {
        knowledgeBaseId: mount.knowledgeBaseId,
        knowledgeBaseName: mount.base.name,
...
function snapshotTurn(session, turn, maxChars) {
    ...
    for (let index = assistantMessages.length - 1; index >= 0; index -= 1) {
        ...
        if (message !== undefined && messageText(message).length > 0) {
...
async function extractWithLlm(ctx, config, snapshot, mounts, existing, route, writebackPolicy, parentSignal) {
    const defaultScope = config.defaultScope === 'project' && snapshot.projectId !== undefined
        ? { kind: 'project', id: snapshot.projectId }
        : { kind: 'global' };
    const existingBodyLimit = Math.min(4000, Math.max(1000, Math.floor(Math.min(24_000, Math.max(8_000, config.extractionMaxInputChars * .6)) / Math.max(1, existing.length))));
    ...
    const output = await callStructuredModel(ctx, route, message, snapshot.sessionId, config.extractionMaxTokens, config.extractionTimeoutMs, parentSignal, extractionSystemPrompt(writebackPolicy), extractionRetrySystemPrompt(writebackPolicy), 'extraction');
    ...
    const proposals = items.flatMap((item) => {
        ...
        if (mount === undefined) {
            diagnostics.unknownDestination += 1;
            return [];
        }
        ...
        if (item.action !== 'create' && targetId === undefined) {
            diagnostics.invalid += 1;
            return [];
        }
        ...
        if (item.action === 'finalize') {
            ...
            try {
                const proposal = lifecycleProposal(target, { state: item.state, confirmation: item.confirmation, note: item.note, wholeDocument: true }, snapshot.userText, { sessionId: snapshot.sessionId, turn: snapshot.turn, ...snapshot.assistantMessageId === undefined ? {} : { messageId: snapshot.assistantMessageId } });
        ...
        const draft = {
            ...
            tags: normalizeTags([
                ...
                ...mount.includeTags,
                ...(Array.isArray(item.tags) ? item.tags.filter((tag) => typeof tag === 'string') : []),
...
async function callExtractionModel(ctx, route, message, sessionId, maxTokens, timeoutMs, parentSignal, system, reasoningEffort) {
    ...
    try {
        for await (const chunk of ctx.llm.stream({
            ...
            if (chunk.type === 'text-delta' && 'text' in chunk && typeof chunk.text === 'string')
                deltas += chunk.text;
            if (chunk.type === 'block-end' && 'block' in chunk && chunk.block.type === 'text')
                completedText += chunk.block.text ?? '';
            if (chunk.type === 'finish' && 'reason' in chunk)
                finish = chunk.reason;
        }
    }
    finally {
...
async function callStructuredModel(ctx, route, message, sessionId, maxTokens, timeoutMs, parentSignal, system, retrySystem, operation) {
    const first = await callExtractionModel(ctx, route, message, sessionId, maxTokens, timeoutMs, parentSignal, system);
    ...
    const retry = await callWithLowReasoningFallback(ctx, route, message, sessionId, retryBudget, timeoutMs, parentSignal, retrySystem);
...
async function callWithLowReasoningFallback(ctx, route, message, sessionId, maxTokens, timeoutMs, parentSignal, system) {
    const reasoningEffortRejection = /reasoning|unsupported|unknown (?:field|option|parameter)|invalid (?:field|option|parameter)/i;
    try {
        const result = await callExtractionModel(ctx, route, message, sessionId, maxTokens, timeoutMs, parentSignal, system, 'low');
        if (result.finish?.kind === 'error'
            ...
            return callExtractionModel(ctx, route, message, sessionId, maxTokens, timeoutMs, parentSignal, system);
        }
        return result;
    }
    catch (error) {
        const messageText = error instanceof Error ? error.message : String(error);
        if (!reasoningEffortRejection.test(messageText))
            throw error;
        ctx.logger.warn(`dsh-knowledge: ${route.provider}/${route.model} does not accept reasoningEffort; retrying without it`);
        return callExtractionModel(ctx, route, message, sessionId, maxTokens, timeoutMs, parentSignal, system);
    }
...
function extractionSystemPrompt(policy) {
    return `${EXTRACTION_SYSTEM_PROMPT}\n\n${policy === 'conservative' ? CONSERVATIVE_POLICY_PROMPT : PROACTIVE_POLICY_PROMPT}`;
}
function extractionRetrySystemPrompt(policy) {
...
function qualifiesForWriteback(policy, retention, confidence) {
    if (!isRecord(retention) || retention.durable !== true)
        return false;
    if (policy === 'conservative')
        return confidence >= .85;
    return confidence >= .65;
}
function qualifiesForDirectWrite(proposal, policy) {
    if (proposal.action === 'conflict')
        return false;
    if (proposal.change?.kind === 'finalize')
        return proposal.draft.source?.evidence === 'explicit' && proposal.draft.confidence >= .95;
    ...
    if (policy === 'conservative')
        return proposal.draft.confidence >= .9;
...
function deduplicateExistingEntries(entries) {
    return [...new Map(entries.map(entry => [entry.id, entry])).values()];
}
function coalesceDocumentProposals(proposals) {
    const groups = new Map();
    ...
    for (const proposal of proposals) {
        if (proposal.change?.kind === 'finalize' && proposal.targetId && editingTargets.has(proposal.targetId))
            continue;
    ...
    return [...groups.values()].flatMap(group => {
        ...
        if (closures.length > 0) {
            ...
            if (edits.length > 0)
                return coalesceDocumentProposals(edits);
...
function mergeDocumentProposals(current, proposal) {
    const targetId = current.targetId ?? proposal.targetId;
    let action = current.action === 'conflict' || proposal.action === 'conflict'
        ? 'conflict'
...
function mergeCandidateChanges(current, proposal) {
    ...
    const leftRevision = left;
    const rightRevision = right;
...
function mergeOptionalSections(left, right) {
    if (!left?.trim())
        return right?.trim() || undefined;
    if (!right?.trim())
        return left.trim();
    return mergeDocumentSections(left, right);
}
function documentSection(body, sectionTitle) {
    const content = body.trim();
    const title = sectionTitle?.trim().replace(/^#+\s*/u, '');
    if (!title || /^#{1,6}\s/mu.test(content))
        return content;
    return `## ${title}\n\n${content}`;
}
function parseModelDocumentChange(item, target) {
    if (target === undefined) {
    ...
    if (rawChange?.kind === 'revise') {
        if (!Array.isArray(rawChange.edits) || rawChange.edits.length < 1 || rawChange.edits.length > 20)
            return undefined;
        const edits = [];
        for (const rawEdit of rawChange.edits) {
            ...
            if (!oldText || oldText.length > 12_000 || newText.length > 12_000)
                return undefined;
...
function relevantBodyExcerpt(body, query, maxChars) {
...
function mergeDocumentSections(current, incoming) {
    const left = current.trim();
    const right = incoming.trim();
    const leftKey = normalizedMarkdown(left);
    const rightKey = normalizedMarkdown(right);
    if (!rightKey || leftKey === rightKey || leftKey.includes(rightKey))
        return left;
    if (!leftKey || rightKey.includes(leftKey))
        return right;
...
function strongerEvidence(left, right) {
    const rank = { inferred: 1, verified: 2, explicit: 3 };
    if (left === undefined)
        return right;
    if (right === undefined)
        return left;
...
function parseScope(value, fallback, projectId) {
    if (!isRecord(value))
        return fallback;
    if (value.kind === 'global')
        return { kind: 'global' };
    if (value.kind === 'project' && projectId !== undefined)
...
async function proposeUnlessFinalized(provider, proposal, sourceKey, signal) {
    try {
        return await provider.propose(proposal, sourceKey, signal);
    }
    catch (error) {
        if (/\bis finalized as\b/i.test(errorMessage(error)))
            return undefined;
        throw error;
    }
}
