...
export function normalizeFinalizationChange(value) {
    const input = value;
    if (!input || input.kind !== 'finalize' || (input.state !== 'resolved' && input.state !== 'complete')
        ...
        || typeof input.confirmation !== 'string' || !input.confirmation.trim() || input.confirmation.length > 2000)
        throw new Error('文档结束候选缺少有效状态、版本或确认依据');
    return { kind: 'finalize', state: input.state, baseVersion: input.baseVersion, baseHash: input.baseHash, note: input.note.trim(), confirmation: input.confirmation.trim() };
}
/** Ground status changes in a current direct user confirmation, not an Agent claim. */
export function assertLifecycleConfirmation(text, quote, state) {
    const normalized = text.normalize('NFKC').replace(/\s+/gu, ' ').trim();
    const evidence = typeof quote === 'string' ? quote.normalize('NFKC').replace(/\s+/gu, ' ').trim() : '';
    if (!evidence || evidence.length > 2000 || !normalized.includes(evidence))
        throw new Error('请引用本轮用户明确确认解决或结束的原话');
    ...
    if (/[?？]/u.test(clause) || /(?:是否|能否|是不是|好像|似乎|可能|也许|应该|大概|要是)/u.test(clause)
        ...
        throw new Error('疑问或不确定表述不能作为结束确认');
    if (/(?:不代表|不表示|不意味着|不能算|不确定|不知道|未确认|没有确认|一旦|等到|假设|如果|假如|要是).{0,80}(?:解决|修复|完成|结束)|\b(?:wasn't|wasn’t|isn’t|isn't|hasn't|hasn’t|if|unless)\b.{0,80}\b(?:resolved|fixed|completed|finished)\b/iu.test(normalized))
    ...
    if (!confirmed)
        throw new Error('需要用户明确确认“已解决”或“收集完成／结束”，普通“好了”不自动封存知识');
}
export function lifecycleProposal(entry, request, userText, source) {
    assertLifecycleConfirmation(userText, request.confirmation, request.state);
    if (entry.documentState !== 'open')
        throw new Error('文档已经结束；如需继续修改，请先在知识库中重新打开');
    if (typeof request.wholeDocument !== 'boolean')
        throw new Error('必须明确区分整篇结束与局部问题解决');
    if (typeof request.note !== 'string' || !request.note.trim() || request.note.length > 2000)
        throw new Error('请提供最多 2000 字符的解决结论');
    const note = request.note.trim();
    const base = { baseVersion: entry.version, baseHash: contentHash(entry) };
    const draft = { knowledgeBaseId: entry.knowledgeBaseId, title: entry.title, body: entry.body, type: entry.type, tags: entry.tags, scope: entry.scope, confidence: 1, source: { ...source, evidence: 'explicit' } };
    if (request.wholeDocument) {
        if (request.oldText !== undefined || request.newText !== undefined)
            throw new Error('整篇结束仅修改状态和结论，不接受正文替换；局部修改请指定 wholeDocument=false');
    ...
    if (typeof request.oldText !== 'string' || !request.oldText || request.oldText.length > 12000 || typeof request.newText !== 'string' || !request.newText.trim() || request.newText.length > 12000)
        throw new Error('局部状态更新需要唯一的 oldText 和包含状态、结论的 newText（各最多 12000 字符）');
    if (request.oldText.trim() === entry.body.trim())
        throw new Error('局部更新不能替换整篇文档，请定位对应的问题段落');
