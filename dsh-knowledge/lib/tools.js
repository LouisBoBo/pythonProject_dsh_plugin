...
import { formatKnowledgeEntry, formatKnowledgeBaseMatches, formatSearchResults, readMountedKnowledge, resolveRecallMounts, searchMountedKnowledgeBases, searchMountedKnowledge, selectMounts, } from './retrieval.js';
...
export function registerKnowledgeTools(ctx, provider, codec, noteCodec) {
...
function searchKnowledgeBaseTool(provider) {
    return {
        ...
        async execute(raw, exec) {
            ...
            if (mounts.length === 0)
                return 'No knowledge bases are mounted for recall in this session.';
            return formatKnowledgeBaseMatches(query, searchMountedKnowledgeBases(mounts, query, limit));
        },
    };
}
function createKnowledgeBaseTool(provider, llm) {
    return {
...
function updateKnowledgeBaseTool(provider, llm) {
    return {
        ...
        async execute(raw, exec) {
            ...
            if (args.name !== undefined)
                patch.name = requireNonEmptyString(args.name, 'name', 100);
            if (args.description !== undefined)
                patch.description = optionalTrimmedString(args.description, 'description', 2000);
            if (args.defaultTags !== undefined)
                patch.defaultTags = normalizeTags(optionalStringArray(args.defaultTags, 'defaultTags', 32, 100));
            if (args.extractionInstructions !== undefined) {
                patch.extractionInstructions = optionalTrimmedString(args.extractionInstructions, 'extractionInstructions', 4000);
            }
            const writebackPolicy = parseWritebackPolicy(args.writebackPolicy);
            if (writebackPolicy !== undefined)
                patch.writebackPolicy = writebackPolicy;
            ...
            if (Object.keys(patch).length === 0)
                throw new Error('knowledge_base_update requires at least one field to modify');
            if (provider.mode !== storage)
                throw new Error('the active knowledge storage changed while resolving the base; retry the update');
...
function searchTool(provider, codec) {
    return {
...
function readTool(provider, codec) {
    return {
        name: 'knowledge_read',
...
function parseWritebackPolicy(value) {
    if (value === undefined)
        return undefined;
...
async function resolveKnowledgeBase(provider, requested, signal) {
    ...
    if (matches.length === 0)
        throw new Error(`knowledge base ${JSON.stringify(requested)} was not found in the active ${provider.mode} storage`);
    if (matches.length > 1)
        throw new Error(`knowledge base ${JSON.stringify(requested)} is ambiguous; use its exact id`);
    return matches[0];
}
async function validateWritebackRoute(llm, provider, model, signal) {
    try {
...
function formatKnowledgeBaseMutation(storage, operation, base, warning) {
...
function requireAgent(exec) {
    if (exec.agent === undefined)
        throw new Error('knowledge tools require a calling DSH agent');
    return exec.agent;
}
...
function requireNonEmptyString(value, name, maxLength) {
