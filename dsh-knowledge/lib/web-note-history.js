...
export function createNoteHistoryView(options) {
    ...
    function putCache(version, value) {
    ...
    function syncVersionButtons() {
        for (const button of versionList.querySelectorAll('[data-note-version]')) {
    ...
    async function selectVersion(version) {
        ...
        try {
            ...
            if (destroyed || currentRequest !== request || (error instanceof DOMException && error.name === 'AbortError'))
                return;
    ...
    for (const version of options.versions) {
        ...
        button.append(node('strong', '', `版本 ${version.version}${current ? ' · 当前' : ''}`), node('span', '', options.formatDate(version.createdAt)), node('small', '', options.formatBytes(version.size)));
        button.addEventListener('click', () => { void selectVersion(version); });
        versionList.append(button);
    }
    restoreButton.addEventListener('click', async () => {
        if (!selected || !contentLoaded || restoreButton.disabled)
            return;
    ...
    if (selected)
        void selectVersion(selected);
    else
        content.replaceChildren(node('div', 'note-history-error', '还没有可查看的保存记录。'));
    return {
        element: root,
        destroy: () => {
            destroyed = true;
...
//# sourceMappingURL=web-note-history.js.map