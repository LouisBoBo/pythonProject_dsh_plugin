...
export function migrateKnowledgeDatabase(db, notes) {
    ...
    if (version === 10) {
        ...
        try {
            ...
            const candidateColumns = candidateTable === undefined
                ? []
                : db.prepare('PRAGMA table_info(knowledge_candidates)').all();
  ...
  `);
    ...
    if (version === 12) {
        ...
        `);
                ...
                const extractionColumns = db.prepare('PRAGMA table_info(extraction_jobs)').all();
    ...
    if (version === 13) {
        ...
        try {
            ...
            if (!columns.some(column => column.name === 'group_name'))
                db.exec("ALTER TABLE knowledge_bases ADD COLUMN group_name TEXT NOT NULL DEFAULT ''");
...
function backfillLegacyNoteReferences(db, notes) {
    const insert = db.prepare(`
    INSERT OR IGNORE INTO knowledge_note_references(knowledge_id,note_id,source,source_session_id,created_at)
    VALUES(?,?,'legacy',NULL,?)
  `);
    const timestamp = nowIso();
    const rows = db.prepare("SELECT id,body FROM knowledge_entries WHERE body LIKE '%note://note_%'").all();
