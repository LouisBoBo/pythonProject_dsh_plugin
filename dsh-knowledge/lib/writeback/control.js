...
export function registerWritebackControl(ctx, queue) {
    return ctx.webServer?.register({
        ...
        handler(req, res) {
            try {
                ...
                if (!queue)
                    throw Object.assign(new Error('知识库回写已停用'), { status: 409 });
                const url = new URL(req.url ?? '/', 'http://localhost');
                let result;
                if (req.method === 'GET') {
                    const offset = Number(url.searchParams.get('offset') ?? 0);
                    if (!Number.isSafeInteger(offset) || offset < 0)
                        throw Object.assign(new Error('无效分页参数'), { status: 400 });
                    const status = url.searchParams.get('status') || undefined;
                    if (status !== undefined && status !== 'failed')
                        throw Object.assign(new Error('无效状态筛选'), { status: 400 });
                    ...
                    if (!key || !['retry', 'cancel'].includes(action ?? ''))
                        throw Object.assign(new Error('需要任务编号和有效操作'), { status: 400 });
