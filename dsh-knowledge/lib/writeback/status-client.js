...
export class WritebackStatusClient {
    ...
    invalidate() {
        if (this.disposed || !this.visible)
            return;
        if (this.retrying || this.request && !this.request.signal.aborted)
    ...
    async load(retry) {
        ...
        try {
            ...
            if (this.request !== controller || this.disposed)
                return;
            if (!response.ok && (retry || response.status !== 404 || String(body.status) !== 'missing'))
                throw new Error(body.error ?? `状态请求失败（HTTP ${response.status}）`);
            if (response.ok && !body.summary)
                throw new Error('无效的回写状态响应');
            if (response.ok && body.summary) {
