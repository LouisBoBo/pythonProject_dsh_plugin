...
const retryDelay = (attempt) => [5_000, 15_000, 60_000, 180_000][Math.min(3, Math.max(0, attempt - 1))];
...
export class WritebackQueue {
    ...
    get revision() {
        ...
        const external = this.db.prepare('PRAGMA data_version').get();
        const local = this.db.prepare('SELECT total_changes() AS changes').get();
    ...
    notify() {
        for (const listener of this.listeners) {
            try {
                ...
                this.warn(`writeback status notification: ${String(error)}`);
            }
        }
    }
    constructor(path, execute, warn = () => { }) {
      ...
      INSERT OR IGNORE INTO queue_lease VALUES(1,'',0);`);
        ...
        try {
            const columns = this.db.prepare('PRAGMA table_info(queue_jobs)').all();
            if (!columns.some(column => column.name === 'cancel_requested'))
                this.db.exec('ALTER TABLE queue_jobs ADD COLUMN cancel_requested INTEGER NOT NULL DEFAULT 0');
            if (!columns.some(column => column.name === 'created_at'))
                this.db.exec('ALTER TABLE queue_jobs ADD COLUMN created_at INTEGER');
    ...
    enqueue(work) {
        if (this.closed)
            throw new Error('知识库回写队列已关闭');
        const existing = this.status(work.snapshot.sourceKey);
        if (existing)
            return existing;
        const count = this.db.prepare("SELECT count(*) AS count FROM queue_jobs WHERE status NOT IN ('completed','cancelled')").get();
        if (count.count >= 1000)
            throw new Error('回写队列已满，请先处理失败任务；未删除任何未完成记录');
        const view = { status: 'queued', summary: '知识库回写 · 等待回写', retryable: false };
    ...
    completeEmpty(sourceKey, sessionId) {
        const view = { status: 'completed', summary: '知识库回写 · 当前回答无可提取内容', retryable: false };
    ...
    status(key) {
        const row = this.db.prepare('SELECT id,session_id,status,view FROM queue_jobs WHERE source_key=?').get(key);
        if (!row)
            return undefined;
        const view = JSON.parse(row.view);
        const blocker = row.status === 'queued' ? this.db.prepare("SELECT source_key FROM queue_jobs WHERE session_id=? AND id<? AND status IN ('queued','running') ORDER BY id LIMIT 1").get(row.session_id, row.id) : undefined;
        if (blocker) {
    ...
    retry(key) {
        if (this.closed)
            throw new Error('知识库回写队列已关闭');
        const row = this.db.prepare('SELECT * FROM queue_jobs WHERE source_key=?').get(key);
        if (!row)
            throw Object.assign(new Error('旧记录没有持久化内容快照，不能安全重试'), { status: 409 });
        if (row.status !== 'failed')
            return this.status(key);
        if (!row.payload)
            throw Object.assign(new Error('回写内容快照不可用'), { status: 409 });
        const view = { status: 'queued', summary: '知识库回写 · 已排队重试', retryable: false };
    ...
    list(sessionId = '', offset = 0, limit = 50, status) {
        ...
        const total = this.db.prepare(`SELECT count(*) AS count FROM queue_jobs ${where}`).get(...args).count;
        const rows = this.db.prepare(`SELECT source_key,session_id,attempts,created_at FROM queue_jobs ${where} ORDER BY id DESC LIMIT ? OFFSET ?`).all(...args, limit, offset);
    ...
    cancel(key) {
        ...
        try {
            ...
            if (!current)
                throw Object.assign(new Error('回写任务不存在'), { status: 404 });
        ...
        if (this.activeKey === key)
            this.controller?.abort(new Error('用户取消回写'));
        this.kick();
        return this.status(key);
    }
    finishCancellation(key) {
        const view = { status: 'cancelled', summary: '知识库回写 · 已取消；已写入的内容不会撤销', retryable: false };
    ...
    get isRunning() { return this.controller !== undefined; }
    kick() {
        if (!this.closed)
            this.notify();
        ...
        this.active = new Promise(resolve => setImmediate(resolve)).then(() => this.drain())
    ...
    claim() {
        ...
        try {
            const now = Date.now();
            const lease = this.db.prepare('SELECT owner,expires FROM queue_lease WHERE id=1').get();
            if (lease.owner !== this.owner && lease.expires > now) {
                this.db.exec('COMMIT');
                return undefined;
            }
            // Only the current lease owner may recover work abandoned by a dead process.
            for (const cancelled of this.db.prepare("SELECT source_key FROM queue_jobs WHERE status='running' AND cancel_requested=1").all())
                this.finishCancellation(cancelled.source_key);
    ...
    async drain() {
        while (!this.closed) {
            ...
            const assertLease = () => {
                ...
                const lease = this.db.prepare('SELECT owner,expires FROM queue_lease WHERE id=1').get();
                if (lease.owner !== this.owner || lease.expires < Date.now())
                    throw new Error('回写工作租约已失效');
                if (this.db.prepare('SELECT 1 FROM queue_jobs WHERE id=? AND cancel_requested=1').get(row.id))
                    throw new Error('用户取消回写');
            };
            ...
            renewal.unref?.();
            try {
                let plan = row.plan ? JSON.parse(row.plan) : undefined;
                const view = await this.execute(JSON.parse(row.payload), {
                    load: () => plan,
                ...
                if (view.status !== 'completed')
                    throw new Error(view.error ?? '回写尚未确认完成');
                ...
                const lease = this.db.prepare('SELECT owner FROM queue_lease WHERE id=1').get();
