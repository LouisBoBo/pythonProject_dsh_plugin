...
export class KnowledgeHandleCodec {
    ...
    constructor(secret) {
        this.secret = secret;
        if (secret.length < 32)
            throw new Error('knowledge handle secret must contain at least 32 bytes');
    }
    encode(sessionId, entry) {
    ...
    decode(handle, sessionId) {
        const parts = handle.trim().split('.');
        if (parts.length !== 3 || parts[0] !== 'k1')
...
export async function resolveRecallMounts(provider, agent, signal) {
...
export async function resolveKnowledgeMounts(provider, agent, signal) {
...
export async function searchMountedKnowledge(provider, agent, mounts, query, limit, codec, signal) {
...
export async function readMountedKnowledge(provider, agent, handle, codec, signal) {
    ...
    if (mount === undefined)
        throw new Error('knowledge entry is outside the mounted recall scope');
...
export function formatMountCatalog(mounts, maxChars, _writebackPolicy) {
    if (mounts.length === 0)
        return '';
    let output = [
    ...
    for (const mount of mounts) {
        ...
        if (output.length + item.length > maxChars)
            break;
        output += item;
        shown++;
    }
...
export function selectAutomaticRecallHits(hits, limit, minScore) {
    const boundedLimit = Math.max(0, Math.min(Math.trunc(limit), 10));
    if (boundedLimit === 0)
        return [];
    const unique = new Map();
    for (const hit of hits) {
        if (!Number.isFinite(hit.score) || hit.score < minScore)
            continue;
        const current = unique.get(hit.entry.id);
        if (current === undefined || hit.score > current.score)
            unique.set(hit.entry.id, hit);
    }
    return [...unique.values()]
        .sort((left, right) => right.score - left.score || right.entry.updatedAt.localeCompare(left.entry.updatedAt))
        .slice(0, boundedLimit);
}
export function formatAutomaticRecall(hits, maxChars) {
    if (hits.length === 0)
        return '';
    let output = [
...
export function searchMountedKnowledgeBases(mounts, query, limit) {
...
export function formatKnowledgeBaseMatches(query, matches) {
    if (matches.length === 0) {
...
export function formatSearchResults(query, hits) {
    if (hits.length === 0) {
...
export function selectMounts(mounts, requestedBase) {
    const value = requestedBase.trim();
    if (!value)
        throw new Error('knowledge_search requires an exact mounted knowledge-base id or name');
    ...
    if (selected.length === 0)
        throw new Error(`knowledge base ${JSON.stringify(value)} is not mounted for recall`);
    return selected;
}
function scoreMountedBase(mount, normalizedQuery, units) {
    const fields = [
...
function entryMatchesMount(entry, mount, projectId) {
    if (entry.scope.kind === 'project' && entry.scope.id !== projectId)
        return false;
    if (mount.includeTags.length > 0 && !entry.tags.some(tag => mount.includeTags.includes(tag)))
        return false;
    if (entry.tags.some(tag => mount.excludeTags.includes(tag)))
        return false;
    return true;
}
