...
export interface RuntimeContextLike {
    ...
    on(name: 'agent/pre-step', listener: (payload: PreStepPayload, next: () => Promise<PreStepDecision>) => Promise<PreStepDecision>): () => void;
    on(name: 'agent/turn-stopping', listener: (payload: {
        agent: AgentLike;
        turn: number;
        signal: AbortSignal;
    }) => void | Promise<void>): () => void;
    on(name: 'system-prompt/assemble', listener: (assembly: PromptAssemblyLike, context: AssembleContextLike, next: () => Promise<PromptAssemblyLike>) => Promise<PromptAssemblyLike>): () => void;
...
export declare function messageText(message: MessageLike): string;
export declare function createRecallMessage(text: string): MessageLike;
