...
export function registerKnowledgeNoteTools(ctx, provider, codec, knowledgeCodec) {
...
async function authorizeReferencedNote(provider, codec, exec, knowledgeHandle, noteId) {
    const agent = requireToolAgent(exec, 'knowledge note tools');
    ...
    if (!references.some(reference => reference.note.id === noteId))
        throw new Error('note is not referenced by this mounted knowledge document');
}
function readNoteTool(provider, codec, knowledgeCodec) {
    return {
        ...
        async execute(raw, exec) {
            ...
            if (args.knowledgeHandle !== undefined)
                await authorizeReferencedNote(provider, knowledgeCodec, exec, args.knowledgeHandle, node.id);
            else
                assertExplicitKnowledgeNoteRequest(agent, 'inspect');
...
function updateNoteTool(provider, codec, knowledgeCodec) {
    return {
        ...
        async execute(raw, exec) {
            ...
            const node = await resolveNoteHandle(provider, codec, agent.session.id, handle, exec.signal);
            if (args.knowledgeHandle !== undefined && operation !== 'rename')
                await authorizeReferencedNote(provider, knowledgeCodec, exec, args.knowledgeHandle, node.id);
            else
            ...
            if (operation === 'rename') {
                ...
                if (expectedVersion === undefined)
                    throw new Error('请先用 knowledge_note_read 读取笔记，并提供返回的 note.version 作为 expectedVersion');
                if (node.version !== expectedVersion)
                    throw new Error('笔记已变化，请重新读取并合并后更新');
