...
export function inspectSensitiveContent(content) {
    ...
    if (/-----BEGIN\s+(?:(?:RSA|EC|DSA|OPENSSH)\s+)?PRIVATE KEY-----/iu.test(content))
        findings.add('private-key');
    if (/\bauthorization\s*:\s*(?:basic|bearer)\s+[^\s"'<>]{8,}/iu.test(content))
        findings.add('authorization-header');
    if (/^(?:set-cookie|cookie)\s*:\s*\S.{6,}$/imu.test(content))
        findings.add('cookie');
    if (/(?:https?|ssh):\/\/[^\s/:@]+:[^\s/@]{4,}@/iu.test(content))
        findings.add('embedded-url-credential');
    ...
    if (containsCredentialAssignment(content))
