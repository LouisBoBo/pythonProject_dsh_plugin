...
export const Config = Schema.object({
    ...
    extractionEnabled: Schema.boolean().default(false),
...
export function resolveConfig(config) {
    ...
    if (resolved.backend === 'remote') {
        ...
        if (resolved.remoteToken.trim().length < 24)
            throw new Error('remoteToken must contain at least 24 characters');
        if (resolved.exposeApi)
            throw new Error('remote knowledge backend cannot expose the local API');
    }
...
export function normalizeTrustedShareOrigins(values) {
    ...
    for (const raw of values) {
        ...
        try {
            ...
            throw new Error(`invalid trusted share origin: ${value}`);
        }
...
function deriveConnectionPath(databasePath) {
...
function normalizePrefix(value) {
    ...
    if (!/^\/[a-zA-Z0-9/_-]+$/.test(prefix))
        throw new Error('apiPrefix must be an absolute URL path');
    return prefix;
}
