 ...
 */
export function assertExplicitKnowledgeBaseManagementRequest(agent, operation) {
...
export function explicitlyRequestsKnowledgeBaseManagement(input, operation) {
...
export function assertExplicitKnowledgeNoteReferenceRequest(agent, operation) {
...
export function assertExplicitKnowledgeNoteRequest(agent, operation, target = 'any') {
    const text = currentDirectUserText(agent).normalize('NFKC').toLocaleLowerCase('zh-CN').trim();
    const clauses = text.split(/[。！？!?；;\n，,]+/u).map(clause => clause.trim()).filter(Boolean);
...
export function explicitlyRequestsKnowledgeNote(input, operation, target = 'any') {
    const text = input.normalize('NFKC').toLocaleLowerCase('zh-CN').trim();
    if (text.length === 0)
        return false;
    return text.split(/[。！？!?；;\n，,]+/u)
...
function clauseRequestsKnowledgeBaseManagement(text, operation) {
    if (operation === 'create') {
...
function clauseRequestsKnowledgeNoteReference(text, operation) {
    const subject = /(?:知识|知識|knowledge).{0,28}(?:笔记|筆記|note)|(?:笔记|筆記|note).{0,28}(?:知识|知識|knowledge)/iu.test(text);
    if (!subject)
        return false;
    if (operation === 'inspect')
        return /(?:引用|关联|關聯|连接|連接|link|reference|associate|attach)/iu.test(text);
    if (operation === 'add') {
        if (/(?:不要|别|取消|移除|删除|解除|do\s+not|don't|remove|delete|detach|unlink).{0,20}(?:引用|关联|link|reference|attach)/iu.test(text))
            return false;
...
function clauseRequestsKnowledgeNote(text, operation, target) {
    ...
    if (operation === 'inspect')
        return true;
    if (deniesKnowledgeNoteMutation(text, operation))
        return false;
...
function deniesKnowledgeNoteMutation(text, operation) {
    const verbs = {
...
function deniesKnowledgeBaseMutation(text, operation) {
