...
export function createNoteShareManifest(share, nodes, truncated) {
...
export async function inspectNoteShareUrl(rawUrl, store, policy = {}) {
    ...
    return { url: url.href, manifest: validateManifest(JSON.parse(payload.toString('utf8'))) };
}
export async function importNoteShare(store, rawUrl, parentId, policy = {}) {
    ...
    if (localShare !== undefined) {
        ...
        if (nodes.length > MAX_IMPORT_NODES)
            throw inputError('分享目录超过 500 项，暂时不能导入');
    ...
    if (manifest.nodes.length === 0)
        throw inputError('分享中没有可导入的内容');
    const rootManifest = manifest.nodes.find(node => node.path === manifest.share.name);
    if (rootManifest === undefined || rootManifest.kind !== manifest.share.kind)
        throw inputError('分享清单缺少有效的根项目');
    ...
    try {
        ...
        for (const item of ordered) {
            ...
            if (parentPath !== null && destinationParentId === undefined)
                throw inputError(`分享清单中的父目录不存在：${parentPath}`);
            let created;
            if (item.kind === 'folder') {
                ...
                if (content.byteLength !== item.size)
                    throw inputError(`分享文件大小校验失败：${item.path}`);
                ...
                if (totalBytes > MAX_IMPORT_TOTAL_BYTES)
                    throw inputError('分享内容超过 256 MiB 导入上限');
            ...
            if (item === rootManifest)
                root = created;
        }
    }
    catch (error) {
        if (root !== undefined)
            await store.delete(root.id).catch(() => { });
        throw error;
    }
    if (root === undefined)
        throw inputError('分享内容导入失败');
    return { root, importedNodes: createdByPath.size, importedFiles, totalBytes };
}
function canonicalShareUrl(raw) {
    let url;
    try {
        url = new URL(raw.trim());
    }
    catch {
        throw inputError('请输入完整的 HTTP 或 HTTPS 分享链接');
    }
    if (url.protocol !== 'https:' && url.protocol !== 'http:')
        throw inputError('分享链接只支持 HTTP 或 HTTPS');
    if (url.username || url.password)
        throw inputError('分享链接不能包含用户名或密码');
    const match = url.pathname.match(/^(.*\/shared\/(share_[A-Za-z0-9_-]{32}))(?:\/)?$/);
    if (match?.[1] === undefined || !SHARE_TOKEN_PATTERN.test(match[2] ?? ''))
        throw inputError('这不是有效的 DSH 笔记分享链接');
    url.pathname = match[1];
    url.search = '';
    url.hash = '';
    return url;
}
function contentUrl(base, noteId) {
    const url = new URL(`${base}/content`);
    url.searchParams.set('noteId', noteId);
    return url;
}
function shareToken(url) { return url.pathname.slice(url.pathname.lastIndexOf('/') + 1); }
async function requestPublicUrl(url, maximumBytes, policy, redirects = 0) {
    if (redirects > 3)
        throw upstreamError('分享服务重定向次数过多');
    ...
    return await new Promise((resolve, reject) => {
        const req = request(url, {
            ...
            if ([301, 302, 303, 307, 308].includes(status)) {
                ...
                if (!location)
                    return reject(upstreamError('分享服务返回了无效重定向'));
                let redirected;
                try {
...
export async function resolveShareTarget(url, policy = {}, lookupHost = lookup) {
    if (url.protocol !== 'https:' && url.protocol !== 'http:')
        throw inputError('分享链接只支持 HTTP 或 HTTPS');
    const hostname = url.hostname.toLocaleLowerCase().replace(/^\[|\]$/g, '');
    if (hostname === 'localhost' || hostname.endsWith('.localhost'))
        throw inputError('分享链接不能指向本机或私有网络');
    const literalFamily = isIP(hostname);
    if (literalFamily !== 0) {
        if (!isPublicAddress(hostname)) {
            if (!isLanAddress(hostname))
                throw inputError('分享链接不能指向本机或敏感网络地址');
    ...
    try {
        ...
        throw upstreamError('无法解析分享链接的主机名');
    }
    ...
    if (!trustedPrivateOrigin) {
        if (!addresses.length || addresses.some(item => !isPublicAddress(item.address) && !isLanAddress(item.address)))
            throw inputError('分享链接不能解析到本机或敏感网络地址');
        requirePrivateShareConfirmation(url, policy);
    }
...
function requirePrivateShareConfirmation(url, policy) {
    if (!isShareResourcePath(url.pathname))
        throw inputError('私有网络访问仅允许笔记分享接口');
    if (policy.confirmedPrivateShareUrl) {
        const approved = canonicalShareUrl(policy.confirmedPrivateShareUrl);
        if (url.origin === approved.origin && [approved.pathname, `${approved.pathname}/manifest`, `${approved.pathname}/content`].includes(url.pathname))
            return;
    }
...
function isShareResourcePath(pathname) {
...
function validateManifest(value) {
    if (!isRecord(value) || value.version !== 1 || !isRecord(value.share) || !Array.isArray(value.nodes))
        throw inputError('分享服务返回的清单格式无效');
    const share = value.share;
    if (typeof share.name !== 'string' || share.name.length === 0 || share.name.length > 255)
        throw inputError('分享名称无效');
    if (!isNoteKind(share.kind) || typeof share.updatedAt !== 'string')
        throw inputError('分享信息无效');
    if (value.nodes.length > MAX_IMPORT_NODES)
        throw inputError('分享目录超过 500 项，暂时不能导入');
    const paths = new Set();
    let totalSize = 0;
    const nodes = value.nodes.map((item, index) => {
        if (!isRecord(item) || typeof item.id !== 'string' || !/^note_[a-f0-9]{32}$/.test(item.id))
            throw inputError(`分享清单第 ${index + 1} 项编号无效`);
        if (typeof item.path !== 'string' || !validRelativePath(item.path))
            throw inputError(`分享清单第 ${index + 1} 项路径无效`);
        if (paths.has(item.path))
            throw inputError(`分享清单存在重复路径：${item.path}`);
        paths.add(item.path);
        if (!isNoteKind(item.kind) || (item.mediaType !== null && typeof item.mediaType !== 'string'))
            throw inputError(`分享清单第 ${index + 1} 项类型无效`);
        const size = item.size;
        if (typeof size !== 'number' || !Number.isSafeInteger(size) || size < 0 || size > MAX_IMPORT_FILE_BYTES)
            throw inputError(`分享清单第 ${index + 1} 项大小无效`);
        if (item.sha256 !== null && (typeof item.sha256 !== 'string' || !/^[a-f0-9]{64}$/.test(item.sha256)))
            throw inputError(`分享清单第 ${index + 1} 项摘要无效`);
        if (item.kind !== 'folder')
            totalSize += size;
    ...
    if (totalSize > MAX_IMPORT_TOTAL_BYTES)
        throw inputError('分享内容超过 256 MiB 导入上限');
    if (share.nodeCount !== nodes.length || share.fileCount !== nodes.filter(node => node.kind !== 'folder').length || share.totalSize !== totalSize)
        throw inputError('分享清单统计信息不一致');
...
function validRelativePath(value) {
    if (value.length === 0 || value.length > 4096 || value.startsWith('/') || value.includes('\\') || value.includes('\0'))
        return false;
    const parts = value.split('/');
    return parts.every(part => part.length > 0 && part.length <= 255 && part !== '.' && part !== '..');
}
function pathDepth(value) { return value.split('/').length; }
function pathName(value) { return value.slice(value.lastIndexOf('/') + 1); }
function pathParent(value) {
    const index = value.lastIndexOf('/');
    return index < 0 ? null : value.slice(0, index);
}
function isNoteKind(value) { return value === 'folder' || value === 'document' || value === 'file'; }
function isRecord(value) { return value !== null && typeof value === 'object' && !Array.isArray(value); }
function inputError(message) { return Object.assign(new Error(message), { status: 400, code: 'BAD_REQUEST' }); }
function upstreamError(message) { return Object.assign(new Error(message), { status: 502, code: 'UPSTREAM_ERROR' }); }
//# sourceMappingURL=share-import.js.map