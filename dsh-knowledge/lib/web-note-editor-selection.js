...
export function createNoteSelectionMenu(options) {
    ...
    function shouldShow() {
        if (destroyed || editor.isDestroyed || options.findIsOpen())
            return false;
    ...
    function positionMenu() {
        ...
        if (destroyed || menu.hidden || editor.isDestroyed)
            return;
    ...
    function onOutsidePointer(event) {
        if (!menu.contains(event.target) && !editor.view.dom.contains(event.target))
            cancelGesture();
    }
    ...
    function onSelectionPointerEnd(event) {
        if (activeSelectionPointerId === undefined || event.pointerId !== activeSelectionPointerId)
            return;
    ...
    function onSelectionPointerCancel(event) {
        if (event.pointerId === activeSelectionPointerId)
            cancelGesture();
    }
