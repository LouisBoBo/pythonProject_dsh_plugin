...
const [apiModule, themeModule, uiModule, modelCatalogModule, dialogModule, selectModule, documentActionsModule, knowledgeImportModule] = await Promise.all([
  ...
  import(moduleUrl('knowledge-import')),
])
...
let knowledgeOriginalUrl = ''

function revokeKnowledgeOriginalUrl() {
  if (!knowledgeOriginalUrl) return
  URL.revokeObjectURL(knowledgeOriginalUrl)
  knowledgeOriginalUrl = ''
}

function knowledgeDocumentPathLabel(document, editor) {
  return editor?.source?.filename || document?.originalFilename || document?.relPath || editor?.title || ''
}
...
function knowledgeOriginalMissingMessage(kind) {
  if (kind === 'docx') return '原 Word 文档不存在，请删除后重新导入'
  return '原 PDF 不存在，请删除后重新导入'
}

function knowledgeOriginalFormatLabel(kind) {
...
async function loadDocumentEditor(workspace, id, signal) {
  ...
  revokeKnowledgeOriginalUrl()
  let originalUrl = ''
  let originalError = ''
  const originalKind = importedOriginalKind(entry.source)
  if (originalKind) {
    try {
      const blob = await binaryRequest(
        originalKind === 'docx'
          ? `entries/${encodeURIComponent(id)}/preview`
          : `entries/${encodeURIComponent(id)}/original`,
        {
          responseType: 'blob',
          accept: originalKind === 'docx' ? 'text/html' : 'application/pdf',
          signal,
        },
      )
      originalUrl = URL.createObjectURL(blob)
      knowledgeOriginalUrl = originalUrl
    } catch {
      originalError = knowledgeOriginalMissingMessage(originalKind)
    }
  }
...
function resolveImportBase(workspace, preferredId) {
  const bases = documentWorkspaceBases(workspace).filter(item => item.status === 'active')
  return bases.find(item => item.id === preferredId) || bases.find(item => item.id === workspace.view.knowledgeBaseId) || bases[0]
}

function knowledgeImportInput(workspace, preferredId) {
  return element('input', {
    ...
    onChange: event => {
      ...
      event.target.value = ''
      void importKnowledgeFiles(workspace, preferredId, files)
    },
  })
}

function knowledgeCreateControl(workspace, preferredId, options = {}) {
  const onClick = () => { void startBlankDocument(workspace, preferredId) }
  if (options.appearance === 'tree') {
    return element('button', { type: 'button', class: 'note-tree-new', title: '新建空白知识文档', onClick },
      element('span', { class: 'note-tree-new-mark', 'aria-hidden': 'true' }, '+'), '新建文档')
  }
  return actionButton('新建文档', onClick, options.variant || 'primary small', { title: '新建空白知识文档' })
}

function knowledgeImportControl(workspace, preferredId, options = 'small') {
  const settings = typeof options === 'string' ? { variant: options } : options
  const input = knowledgeImportInput(workspace, preferredId)
  const open = () => input.click()
  const trigger = settings.appearance === 'tree'
    ? element('button', {
        type: 'button', class: 'note-tree-new',
        title: '导入 Markdown / 纯文本 / PDF / Word 为可检索知识文档',
        onClick: open,
      }, element('span', { class: 'note-tree-new-mark', 'aria-hidden': 'true' }, '+'), '导入文件')
    : actionButton('导入文件', open, settings.variant || 'small', { title: '导入 Markdown / 纯文本 / PDF / Word 为可检索知识文档' })
  return element('div', { class: 'knowledge-import-control' }, trigger, input)
}

async function importKnowledgeFiles(workspace, preferredId, files) {
  if (documentWorkspaceReadOnly(workspace)) return showToast('归档知识库不能导入文档。', 'error')
  const base = resolveImportBase(workspace, preferredId)
  if (!base) return showToast('请先选/建知识库后再导入文件。', 'error')
  const list = [...(files || [])]
  if (!list.length) return
  if (knowledgeImportBusy) return showToast('正在导入，请稍候。')
  knowledgeImportBusy = true
  let created = 0
  let failed = 0
  const reasons = []
  try {
    for (let index = 0; index < list.length; index += 1) {
      ...
      showToast(`正在导入 ${index + 1}/${list.length}`)
      try {
        created += await importOneKnowledgeFile(base.id, file)
      } catch (error) {
        failed += 1
        reasons.push(`${file.name}：${friendlyError(error)}`)
      }
    }
    await reloadDocumentWorkspace(workspace)
    if (workspace.view.knowledgeBaseId !== base.id) {
      workspace.view.knowledgeBaseId = base.id
      workspace.view.expandedBases.add(base.id)
    }
    renderShell()
    const detail = reasons.length ? `。${reasons.slice(0, 3).join('；')}${reasons.length > 3 ? '…' : ''}` : ''
    showToast(`成功 ${created}，失败 ${failed}${detail}`, failed && !created ? 'error' : undefined)
  } catch (error) {
...
async function importOneKnowledgeFile(knowledgeBaseId, file) {
  if (knowledgeImportExtension(file.name) === '.doc') throw new Error('旧版 Word（.doc）请另存为 .docx 后再导入')
  if (!isAllowedKnowledgeImportName(file.name)) throw new Error('仅支持 .md / .txt / .pdf / .docx')
  if (file.size > KNOWLEDGE_IMPORT_FILE_MAX_BYTES) throw new Error('文件过大（上限 16MB）')
  const params = new URLSearchParams({ filename: file.name })
  const result = await binaryUploadRequest(
    `knowledge-bases/${encodeURIComponent(knowledgeBaseId)}/import?${params}`,
    file,
    { method: 'POST', contentType: file.type || 'application/octet-stream' },
  )
  return Number(result?.created) || 0
}

async function downloadKnowledgeOriginal(editor) {
  if (!editor?.id) return
  const blob = await binaryRequest(`entries/${encodeURIComponent(editor.id)}/original?download=1`, {
    responseType: 'blob',
    accept: editor.source?.mediaType || 'application/octet-stream',
  })
  const url = URL.createObjectURL(blob)
  const link = document.createElement('a')
  link.href = url
  link.download = editor.source?.filename || 'document'
  link.click()
...
function activateKnowledgeBaseDropTarget(event, workspace, baseId) {
  if (documentWorkspaceReadOnly(workspace)) return
  if (hasDragType(event, 'Files') && !hasDragType(event, KNOWLEDGE_DOCUMENT_DRAG_TYPE)) {
    event.preventDefault()
...
function dropKnowledgeDocument(event, workspace, targetBaseId) {
  if (hasDragType(event, 'Files') && !hasDragType(event, KNOWLEDGE_DOCUMENT_DRAG_TYPE)) {
    ...
    void importKnowledgeFiles(workspace, targetBaseId, [...(event.dataTransfer?.files || [])])
    return
  }
  ...
  return element('section', { class: 'library-detail', 'aria-labelledby': 'library-detail-title' },
    element('header', { class: 'library-detail-header' },
      ...
      element('div', { class: 'library-detail-actions' },
        ...
        !archived ? knowledgeCreateControl(workspace, base.id, { variant: 'primary small' }) : null,
        !archived ? knowledgeImportControl(workspace, base.id, { variant: 'small' }) : null,
      ),
    ),
...
function renderDocumentWorkspace(workspace, options = {}) {
    ...
    return element('section', {
      ...
      expanded ? element('div', { class: 'note-tree-documents', role: 'group', 'aria-label': `${base.name}文档` },
        ...
        documents.map(document => element('button', {
          ...
          element('strong', { title: document.title }, document.title), element('small', { title: knowledgeDocumentPathLabel(document) }, knowledgeDocumentPathLabel(document))),
  ...
  return element('section', {
    ...
    onDragOver: event => {
      if (readOnly || !hasDragType(event, 'Files') || hasDragType(event, KNOWLEDGE_DOCUMENT_DRAG_TYPE)) return
      event.preventDefault()
      if (event.dataTransfer) event.dataTransfer.dropEffect = 'copy'
    },
    onDrop: event => {
      if (readOnly || !hasDragType(event, 'Files') || hasDragType(event, KNOWLEDGE_DOCUMENT_DRAG_TYPE)) return
      event.preventDefault()
      void importKnowledgeFiles(workspace, view.knowledgeBaseId || activeBases.find(item => item.status === 'active')?.id, [...(event.dataTransfer?.files || [])])
    },
  },
    element('aside', { class: 'note-tree-panel', 'aria-label': '知识目录' },
      element('header', { class: 'note-tree-header' },
        ...
        !readOnly ? element('div', { class: 'note-tree-header-actions note-tree-new-row' },
          knowledgeCreateControl(workspace, view.knowledgeBaseId || activeBases[0]?.id, { appearance: 'tree' }),
          knowledgeImportControl(workspace, view.knowledgeBaseId || activeBases[0]?.id, { appearance: 'tree' }),
        ) : null,
      ),
...
function renderNoteEditor(workspace, editor, base) {
  ...
  if (!editor) return element('main', { class: 'note-editor note-editor-empty' },
    element('div', { class: 'note-empty-content' },
      ...
      !readOnly ? element('div', { class: 'note-empty-actions' },
        knowledgeCreateControl(workspace, view.knowledgeBaseId || state.knowledgeBases.find(item => item.status === 'active')?.id, { variant: 'primary' }),
        knowledgeImportControl(workspace, view.knowledgeBaseId || state.knowledgeBases.find(item => item.status === 'active')?.id, { variant: 'ghost' }),
      ) : null,
    ))
  ...
  const title = element('textarea', {
    class: 'note-title-input', rows: 1, maxlength: 200, placeholder: '无标题文档', 'aria-label': '文档标题',
    onInput: event => {
  ...
  const originalKind = importedOriginalKind(editor.source)
  const viewOriginal = Boolean(originalKind)
  const finalized = !editor.isNew && editor.documentState !== 'open'
  const editable = !finalized && !readOnly && !viewOriginal
  const documentRecord = documentWorkspaceDocuments(workspace).find(item => item.id === editor.id)
  let body
  if (viewOriginal && editor.originalUrl) {
    body = element('iframe', {
      class: 'knowledge-original-frame',
      src: editor.originalUrl,
      title: editor.source.filename,
      ...(originalKind === 'docx' ? { sandbox: '', referrerpolicy: 'no-referrer' } : {}),
    })
  } else if (viewOriginal) {
    body = element('div', { class: 'knowledge-original-missing' }, editor.originalError || knowledgeOriginalMissingMessage(originalKind))
  } else if (editable) {
    body = element('div', { class: 'notes-live-editor knowledge-live-editor', role: 'status', 'aria-label': '正在打开知识文档正文' },
      element('div', { class: 'notes-editor-loading' }, '正在打开文档…'))
  } else {
    body = renderMarkdownPreview(editor.body)
  }
  ...
  return element('main', { class: 'note-editor', 'aria-label': '文档编辑器' },
    element('header', { class: 'note-editor-toolbar' },
      element('div', { class: 'note-breadcrumb' },
        ...
        element('span', { 'aria-hidden': 'true' }, '/'),
        element('strong', { title: editor.isNew ? '新文档' : knowledgeDocumentPathLabel(documentRecord, editor) }, editor.isNew ? '新文档' : knowledgeDocumentPathLabel(documentRecord, editor)),
      ...
      element('div', { class: 'note-editor-actions' },
        finalized ? badge(DOCUMENT_STATE_LABELS[editor.documentState] || '已结束', 'success') : readOnly ? badge('只读') : viewOriginal ? badge(knowledgeOriginalFormatLabel(originalKind)) : null,
        !readOnly && !finalized ? element('span', { class: 'editor-save-status', role: 'status' }, editor.saveState) : null,
        ...
        !readOnly && !editor.isNew ? renderDocumentMenu(editor.title || '知识文档', [
          [
            ...(viewOriginal ? [{ label: '下载原文件', icon: 'download', run: () => { void downloadKnowledgeOriginal(editor) } }] : []),
    ...
    element('div', { class: 'note-editor-scroll', 'data-scroll-key': 'note-editor' },
      ...
      element('article', { class: `note-paper ${viewOriginal ? 'is-original' : `is-${editable ? 'edit' : 'preview'}`}`, 'data-document-mode': viewOriginal ? 'original' : editable ? 'edit' : 'preview' },
        viewOriginal ? null : (editable ? title : element('h1', { class: 'note-preview-title' }, editor.title || '无标题文档')),
        body,
      ),
    ),
    renderDocumentReferenceBar(workspace, editor, editable && !viewOriginal),
    element('footer', { class: 'note-inspector' },
      viewOriginal ? element('span', { class: 'note-format-hint' }, knowledgeOriginalFormatLabel(originalKind)) : finalized || readOnly ? element('span', { class: 'note-format-hint' }, readOnly ? '知识库已归档 · 只读' : '只读封存 · 重新打开后才能编辑') : element('label', {}, element('span', {}, '类型'), element('select', {
      ...
      viewOriginal || finalized || readOnly ? null : element('label', { class: 'note-tags-field' }, element('span', {}, '标签'), element('input', {
